...
/**
 * DevToolsWidget - Combines version indicator and impersonation selector
 * Positioned in bottom-right corner of the dashboard
 */
export default function DevToolsWidget() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 hidden md:flex flex-col items-end gap-2">
      {/* Impersonation Selector - Only shown if user can impersonate */}
      {canImpersonate && <ImpersonationSelector />}
      ...
      >
        <div
          ...
          ${versionExpanded ? 'px-3 py-2' : 'px-2 py-1'}
        `}
        >
          {versionExpanded ? (
